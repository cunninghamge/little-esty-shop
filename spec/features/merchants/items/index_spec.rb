...
RSpec.describe "Items Index" do
  ...
  let(:visit_path) {visit merchant_items_path(@merchant)}
  ...
  describe 'displays' do
    it "merchant's items" do
      ...
      expect(page).to have_link(item1.name, href: merchant_item_path(item1))
    ...
    it "link to create new item" do
      ...
      expect(page).to have_link("New Item", href: new_merchant_item_path)
    end
  end
        ...
        within '#top_items' do
          ...
          top_items.each_with_index do |item, index|
            within "#top-#{item.id}" do
              expect(page).to have_link(item.name, href: merchant_item_path(item))
